...
function Header() {
	global $codigo; // EXEMPLO DE UMA VARIAVEL QUE TERÁ O MESMO VALOR EM QUALQUER ÁREA DO PDF.
       ...
       // $this->Cell(170,15,'INSIRA SEU TEXTO AQUI',0,0,'L'); 
        ...
        $this->SetXY(10,15);
        $this->Cell(190,$l,'Candidatos Classificados Cotistas','B',1,'C');
        ...
        $this->Cell(190,$l,utf8_decode('Classificados Informática Cotistas'),1,0,'C',1);
        $this->Ln();
...
$sql = "SELECT nome, curso, round(sum(((fisica)+(historia)+(portugues)+(matematica))/4),2 ) AS media FROM ALUNO AS a INNER JOIN NOTAS AS n ON (a.idaluno = n.idnotas) WHERE tipoconcorrencia LIKE 'Cota' AND curso LIKE 'Info%' GROUP BY idaluno ORDER BY media DESC";
