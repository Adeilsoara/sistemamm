...
function Header() {
	global $codigo; // EXEMPLO DE UMA VARIAVEL QUE TERÁ O MESMO VALOR EM QUALQUER ÁREA DO PDF.
       ...
       // $this->Cell(170,15,'INSIRA SEU TEXTO AQUI',0,0,'L'); 
        ...
        $this->Cell(190,$l,utf8_decode('Classificados Informática Escola Particular'),1,0,'C',1);
        $this->Ln();
...
$sql = "SELECT nome, curso, round(sum(((fisica)+(historia)+(portugues)+(matematica))/4),2 ) AS media FROM ALUNO AS a INNER JOIN NOTAS AS n ON (a.idaluno = n.idnotas) where categoriaescola like '%Privada%' AND curso LIKE 'Info%' group by idaluno ORDER BY media DESC";
